<?php
/**
 * IMPOT - ITS (Impôt sur Traitements et Salaires)
 */
session_start();
...
// Récupérer la masse salariale
$masseSalariale = 0;
try {
    $salaires = $db->fetchOne("SELECT SUM(montant) as total FROM salaires WHERE client_id = ? AND MONTH(date_paiement) = ? AND YEAR(date_paiement) = ?", [$clientId, $mois, $annee]);
    $masseSalariale = $salaires['total'] ?? 0;
} catch (Exception $e) {
    $masseSalariale = 0;
}

$tauxITS = 10;
$its = round($masseSalariale * $tauxITS / 100);
...
<body class="bg-slate-100 min-h-screen">
    ...
    <div class="bg-white border-b">
        <div class="max-w-7xl mx-auto px-4">
            <nav class="flex space-x-1 overflow-x-auto">
                ...
                <a href="impot-its.php?client=<?=$clientId?>&mois=<?=$mois?>&annee=<?=$annee?>" class="px-4 py-3 text-sm font-medium text-white bg-blue-600 rounded-t-lg whitespace-nowrap">ITS</a>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        <h1 class="text-2xl font-bold text-slate-800 mb-6">ITS - Impôt sur Traitements et Salaires</h1>
                ...
                <table class="w-full max-w-2xl mx-auto">
                    <tbody>
                        <tr class="border-b border-slate-100">
                            <td class="py-4 text-slate-600">Base (Masse salariale)</td>
                            <td class="py-4 text-right">
                                <input type="text" class="w-48 px-3 py-2 border border-slate-300 rounded text-right font-semibold" value="<?= formatMontant($masseSalariale) ?> F CFA" readonly>
                            </td>
                        </tr>
                        <tr class="border-b border-slate-100">
                            <td class="py-4 text-slate-600">Taux applicable</td>
                            <td class="py-4 text-right">
                                <input type="text" class="w-48 px-3 py-2 border border-slate-300 rounded text-right font-semibold bg-slate-50" value="<?= $tauxITS ?> %" readonly>
                            </td>
                        </tr>
                        <tr class="bg-blue-50">
                            <td class="py-4 px-2 text-slate-700 font-semibold">Montant à payer</td>
                            <td class="py-4 px-2 text-right">
                                <input type="text" class="w-48 px-3 py-2 border-2 border-blue-400 bg-blue-50 rounded text-right font-bold text-blue-700" value="<?= formatMontant($its) ?> F CFA" readonly>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div class="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
            <div class="flex items-center">
                ...
                <p class="text-blue-700">L'ITS est calculé sur la masse salariale totale du mois.</p>
            </div>
        </div>
