...
if ($clientId <= 0) {
    header('Location: clients.php?msg=Client introuvable&type=error');
    exit;
}
...
$parametres = $client->getParametresFiscaux();
...
$donnees = [
    'nom' => $client->getNom(),
    'ifu' => $client->getIfu() ?? '',
    'type_activite' => $client->getTypeActivite() ?? '',
    'secteur' => $client->getSecteur(),
    'regime_fiscal' => $client->getRegimeFiscal(),
    'adresse' => $client->getAdresse() ?? '',
    'telephone' => $client->getTelephone() ?? '',
    'email' => $client->getEmail() ?? '',
    'type_tva' => $parametres['type_tva'] ?? 'non_exonere',
    'taux_tva' => $parametres['taux_tva'] ?? '18.00',
    'taux_tva_double' => $parametres['taux_tva_double'] ?? '0',
    'salaires_actif' => $parametres['salaires_actif'] ?? '1',
    'location_actif' => $parametres['location_actif'] ?? '0',
    ...
    'valeur_locative' => $parametres['valeur_locative_mensuelle'] ?? '0',
    'taux_cf' => $parametres['taux_cf'] ?? '3.5',
    'taux_tl' => $parametres['taux_tl'] ?? '1.0',
    'taux_css' => $parametres['taux_css'] ?? '0.5',
    'taux_irf' => $parametres['taux_irf'] ?? '12.0',
    'taux_tf' => $parametres['taux_tf'] ?? '3.0'
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Si pas d'erreurs, mettre à jour le client
    if (empty($erreurs)) {
        try {
            ...
            if ($client->sauvegarder()) {
                ...
                // Redirection
                header('Location: clients.php?msg=Client modifié avec succès&type=success');
                exit;
            }
        } catch (Exception $e) {
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <select id="secteur_select" onchange="onSecteurChange(this)" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
                                ...
                                <?php endforeach; ?>
                                <option value="__custom__" <?= !isset($secteurs[$donnees['secteur']]) ? 'selected' : '' ?>>+ Autre (personnalisé)...</option>
                            </select>
                        </div>
                        ...
                        <input type="text" id="secteur_custom" placeholder="Saisir le nom du secteur..."
                               class="w-full mt-2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 <?= !isset($secteurs[$donnees['secteur']]) ? '' : 'hidden' ?>"
