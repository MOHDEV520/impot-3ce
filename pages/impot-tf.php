...
// Récupérer la valeur locative ou base foncière
$baseFonciere = 0;
try {
    $biens = $db->fetchOne("SELECT SUM(valeur_locative) as total FROM biens_fonciers WHERE client_id = ?", [$clientId]);
    $baseFonciere = $biens['total'] ?? 0;
} catch (Exception $e) {
    $baseFonciere = 0;
}

$tauxTF = 15;
$tf = round($baseFonciere * $tauxTF / 100);
...
<body class="bg-slate-100 min-h-screen">
    ...
    <div class="bg-white border-b">
        <div class="max-w-7xl mx-auto px-4">
            <nav class="flex space-x-1 overflow-x-auto">
                ...
                <a href="impot-tf.php?client=<?=$clientId?>&mois=<?=$mois?>&annee=<?=$annee?>" class="px-4 py-3 text-sm font-medium text-white bg-blue-600 rounded-t-lg whitespace-nowrap">TF</a>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        <h1 class="text-2xl font-bold text-slate-800 mb-6">TF - Taxe Foncière</h1>
                ...
                <table class="w-full max-w-2xl mx-auto">
                    <tbody>
                        <tr class="border-b border-slate-100">
                            <td class="py-4 text-slate-600">Base (Valeur locative)</td>
                            <td class="py-4 text-right">
                                <input type="text" class="w-48 px-3 py-2 border border-slate-300 rounded text-right font-semibold" value="<?= formatMontant($baseFonciere) ?> F CFA" readonly>
                            </td>
                        </tr>
                        <tr class="border-b border-slate-100">
                            <td class="py-4 text-slate-600">Taux applicable</td>
                            <td class="py-4 text-right">
                                <input type="text" class="w-48 px-3 py-2 border border-slate-300 rounded text-right font-semibold bg-slate-50" value="<?= $tauxTF ?> %" readonly>
                            </td>
                        </tr>
                        <tr class="bg-blue-50">
                            <td class="py-4 px-2 text-slate-700 font-semibold">Montant à payer</td>
                            <td class="py-4 px-2 text-right">
                                <input type="text" class="w-48 px-3 py-2 border-2 border-blue-400 bg-blue-50 rounded text-right font-bold text-blue-700" value="<?= formatMontant($tf) ?> F CFA" readonly>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div class="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
            <div class="flex items-center">
                ...
                <p class="text-blue-700">La taxe foncière est calculée sur la valeur locative des biens immobiliers.</p>
            </div>
        </div>
